...
```
✨ SKILL ACTIVATED: multi-agent-orchestration
...
6. **Model Tier** — Uses highest-capability model (e.g., `Claude Sonnet 4.5`, `GPT-4`)
...
4. **Tool Selection** — Minimum necessary tools for the role (see copilot-asset-design skill)
...
**Pairs Well With**:
- `copilot-asset-design` — Decision framework for when to use orchestration vs standalone
- `repository-analysis` — Understand repository complexity before choosing pattern
- `implementation-planning` — Strategic planning within orchestrated phases
- `technical-documentation` — Documentation generation within orchestrated workflows
